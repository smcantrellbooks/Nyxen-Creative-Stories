...
def init_db():
    ...
    # Story Settings (World Building) table
    cursor.execute('''
...
# Create the main app
app = FastAPI(title="Nyxen AI Storyteller")
...
# Nyxen System Prompt
NYXEN_SYSTEM_PROMPT = """You are Nyxen, a sophisticated AI storyteller and creative companion developed by S.M. Cantrell.
...
def get_genre_context(genre: str) -> str:
    contexts = {
        "romance": "Focus on emotional depth, chemistry between characters, tender moments, and the journey of connection. Use sensory language that evokes intimacy.",
        "fantasy": "Weave in magical elements, world-building details, and a sense of wonder. Create immersive settings with their own rules and mysteries.",
        "thriller": "Build tension through pacing, create stakes, use cliffhangers. Every detail matters. Keep readers on edge with suspense and unexpected turns.",
        "scifi": "Ground futuristic elements in believable science. Explore how technology affects humanity. Balance world-building with character development."
    }
...
async def generate_image_apifree(prompt: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        if submit_response.status_code != 200:
            raise HTTPException(status_code=submit_response.status_code, detail="Image generation failed to submit")
...
async def generate_tts_apifree(text: str, voice: str = "af_heart") -> str:
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        if response.status_code == 200:
            data = response.json()
            audio_url = data.get("resp_data", {}).get("audio_url")
            if audio_url:
                return audio_url
...
async def generate_video_apifree(prompt: str, image_url: Optional[str] = None) -> str:
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        if submit_response.status_code != 200:
            raise HTTPException(status_code=submit_response.status_code, detail="Video generation failed to submit")
...
@api_router.get("/")
async def root():
    return {"message": "Welcome to Nyxen - AI Storyteller", "developer": "S.M. Cantrell"}
...
async def create_character(data: CharacterCreate):
    ...
    cursor.execute(
        """INSERT INTO characters 
           (id, name, description, traits, backstory, relationships, speech_patterns, emotional_state, created_at, updated_at) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
    ...
    return Character(
        ...
        speech_patterns=data.speech_patterns or "",
        emotional_state=data.emotional_state or "neutral",
        created_at=now,
        updated_at=now
    )
...
async def get_characters():
    ...
    for row in rows:
        characters.append(Character(
            ...
            speech_patterns=row['speech_patterns'] or "",
...
async def update_character(character_id: str, data: CharacterCreate):
    ...
    cursor.execute(
        """UPDATE characters SET 
           name = ?, description = ?, traits = ?, backstory = ?, relationships = ?, 
           speech_patterns = ?, emotional_state = ?, updated_at = ?
           WHERE id = ?""",
